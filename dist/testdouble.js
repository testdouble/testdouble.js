...
module.exports = function(expectedArgs, actualArgs) {
  return _.eq(expectedArgs, actualArgs, function(expected, actual) {
    ...
    return _.all(expectedArgs, function(expectedArg, i) {
      if (_.eq(expectedArg, actualArgs[i])) {
        return true;
      }
      if (_.isFunction(expectedArg != null ? expectedArg.__matches : void 0)) {
        return expectedArg.__matches(actualArgs[i]);
      } else {
        return false;
      }
    });
...
module.exports = {
  ...
  wasInvoked: function(testDouble, args) {
    return this.where(testDouble, args).length > 0;
  },
  where: function(testDouble, args) {
    return _.select(store["for"](testDouble).calls, function(call) {
      return argsMatch(args, call.args);
    });
  },
...
module.exports = function(args, joiner, wrapper) {
  ...
  return _(args).map(function(arg) {
    return "" + wrapper + (stringifyArg(arg)) + wrapper;
  }).join(joiner);
};
