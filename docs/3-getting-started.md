...
providing it to the subject. In this case, we'll just provide a single function
to do the job:
...
describe('MathProblem', function(){
  ...
  it('POSTs a random problem', function(){
    subject.generate()
    console.log(td.explain(createRandomProblem))
  })
})
```

In the above, we use `function([name])` to create a test double function.
Providing a name is completely optional, but improves readability.

### Aside: inspecting a test double

Before we continue, we can get a better understanding by debugging at this
point and invoking `td.explain(createRandomProblem)`. `explain(aTestDouble)` will
return an object describing the calls, stubbings, and invocations made so far
against any test double function, and is a handy way to debug or improve your
understanding of the state of a test at any given point.
...
Because we don't have any meaningful inputs, we'll "stub" (i.e. configure a
response) so that the test double returns the string "some problem":
...
As you can see, `td.when` is invoked, and—as an example to the reader—the
...
double is smart enough to hunt for any methods defined on the function's
prototype, and therefore will return an object that has a test double function
defined on the property `save`. (Note: `td.object()` supports no-arg and named
test doubles as well, so long as your runtime supports ES2015 proxies, which as
of November, 2015, are only supported by FireFox and MS Edge)
...
first test double function, which is a stub. That means that testdouble.js test
...
Speaking of verification, the API of `verify` should look familiar, as it is
